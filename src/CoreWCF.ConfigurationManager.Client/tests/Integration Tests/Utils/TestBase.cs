// Licensed to the .NET Foundation under one or more agreements.
// The .NET Foundation licenses this file to you under the MIT license.

using CoreWCF.ConfigurationManager.Client;
using Microsoft.Extensions.DependencyInjection;
using Microsoft.Extensions.Options;

namespace CoreWCF.ConfigurationManager.Client.Tests.IntegrationTests
{
    public class TestBase
    {
        protected ServiceProvider CreateProvider(string path)
        {
            var services = new ServiceCollection();
            services.AddClientModelServices();
            services.AddClientModelConfigurationManagerFile(path);

            return services.BuildServiceProvider();
        }

        protected static IConfigurationHolder GetConfigurationHolder(ServiceProvider provider)
        {
            //IConfigureOptions<ClientModelOptions> options = provider.GetRequiredService<IConfigureOptions<ClientModelOptions>>();
            //options.Configure(new ClientModelOptions());
            IConfigurationHolder settingHolder = provider.GetService<IConfigurationHolder>();
            return settingHolder;
        }

        //Limiting this logic for test purpose.
        protected static IXmlConfigClientEndpoint GetXmlConfigEndpointByEndpointName(IConfigurationHolder configHolder, string name)
        {
            foreach (var serviceEndPoint in configHolder.ClientEndpoints)
            {
                if (string.Compare(serviceEndPoint.Name, name, true) == 0)
                {
                    return configHolder.GetXmlConfigClientEndpoint(serviceEndPoint);
                }
            }
            throw new System.Exception();//new EndpointNotFoundException();
        }
    }
}
