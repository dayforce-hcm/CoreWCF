...
namespace CoreWCF.ConfigurationManager.Client
{
        ...
        where TServiceModelExtensionElement : ServiceModelExtensionElement
    {
        ...
        public virtual void Add(TServiceModelExtensionElement element)
        {
            if (IsReadOnly())
            {
                throw new ConfigurationErrorsException(SR.Format(SR.ConfigReadOnly));
            }
            if (element == null)
            {
            ...
            if (Contains(element))
            {
                throw new ArgumentException("element", SR.Format(SR.ConfigDuplicateKey, element.ConfigurationElementName));
            }
        ...
        private TServiceModelExtensionElement CreateNewSection(string name)
        {
            ...
            {
                throw new ConfigurationErrorsException(SR.Format(SR.ConfigDuplicateItem,
                    name,
                    GetType().Name),
            ...
            if (null != elementType)
            {
                ...
                {
                    throw new ConfigurationErrorsException(SR.Format(SR.ConfigInvalidExtensionElement,
                        name,
                        CollectionElementBaseType.FullName),
                        ElementInformation.Source,
                        ElementInformation.LineNumber);
            ...
            {
                throw new ConfigurationErrorsException(SR.Format(SR.ConfigInvalidExtensionElementName,
                    name,
                    _extensionCollectionName),
                    ElementInformation.Source,
                    ElementInformation.LineNumber);
        ...
        private Type GetExtensionType(ContextInformation evaluationContext, string name)
        {
            ...
            if (collection.ContainsKey(name))
            {
                ...
                if (null == elementType)
                {
                    throw new ConfigurationErrorsException(SR.Format(SR.ConfigInvalidType, element.Type, element.Name),
                        ElementInformation.Source,
                        ElementInformation.LineNumber);
                }
                return elementType;
