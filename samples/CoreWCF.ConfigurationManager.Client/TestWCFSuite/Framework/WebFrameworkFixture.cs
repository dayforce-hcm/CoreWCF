...
namespace TestWCFSuite.Framework
{
    public class WebFrameworkFixture : IDisposable
    {
        ...
        public WebFrameworkFixture(IMessageSink messageSink) 
        {
            ...
            //run with https
            //dotnet run --no-build --urls "https://localhost:7055;http://localhost:5151" --project TestWCFServiceCore.csproj 
            //run with http only
            //dotnet run --no-build --urls "http://localhost:5151" --project TestWCFServiceCore.csproj 
            var fullProjectPath = Path.Combine(solutionDir, "samples\\CoreWCF.ConfigurationManager.Client", "TestWCFServiceCore");
            ...
            var processConfig = new ProcessConfig
            {
                ...
                ApplicationUrls = "http://localhost:5151"
            };
